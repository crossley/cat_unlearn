...
def make_stim_cats():
    ...
    # Generate stimuli
    category_A_mean = [25, 50]
    category_B_mean = [50, 75]
    category_C_mean = [75, 50]
    category_D_mean = [50, 25]
    ...
    stimuli_C = sample_within_ellipse(category_C_mean, n_stimuli_per_category)
    stimuli_D = sample_within_ellipse(category_D_mean, n_stimuli_per_category)

    # Define the labels
    labels_A = np.array(["A"] * n_stimuli_per_category)
    labels_B = np.array(["B"] * n_stimuli_per_category)
    labels_C = np.array(["C"] * n_stimuli_per_category)
    labels_D = np.array(["D"] * n_stimuli_per_category)

    # Concatenate the stimuli and labels
    stimuli = np.concatenate([stimuli_A, stimuli_B, stimuli_C, stimuli_D])
