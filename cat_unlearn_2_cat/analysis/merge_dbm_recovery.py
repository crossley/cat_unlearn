...
def merge_recovery_chunks(in_dir, block, out_dir, expected_num_chunks):
    pattern = f"fit_dbm_recovery_block_{block}_chunk_*.csv"
    paths = sorted(glob.glob(os.path.join(in_dir, pattern)))
    if len(paths) == 0:
        raise FileNotFoundError(f"No recovery chunks found in {in_dir}")
    ...
    out_prefix = os.path.join(out_dir, f"fit_dbm_recovery_block_{block}")
